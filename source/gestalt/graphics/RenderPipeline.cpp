...
namespace gestalt::graphics {
  void RenderPipeline::init(const std::shared_ptr<IGpu>& gpu, const std::shared_ptr<Window>& window,
        ...
        for (auto& image_attachment : resource_registry_->attachment_list_) {
          ...
          if (image_attachment.image->getType() == TextureType::kColor) {
            ...
            vkCmdClearDepthStencilImage(
                cmd, image_attachment.image->image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
    ...
    void RenderPipeline::execute(const std::shared_ptr<RenderPass>& render_pass, VkCommandBuffer cmd) {
    ...
    bool RenderPipeline::acquire_next_image() {
      ...
      VkResult e = vkAcquireNextImageKHR(gpu_->getDevice(), swapchain_->swapchain, UINT64_MAX,
                                  frame().swapchain_semaphore, nullptr,
    ...
    void RenderPipeline::present(VkCommandBuffer cmd) {
      ...
      VkSemaphoreSubmitInfo waitInfo
          = vkinit::semaphore_submit_info(VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT_KHR, frame().swapchain_semaphore);
