...
public:
  void init(const std::shared_ptr<ResourceManager>& resource_manager,
            const std::shared_ptr<Repository>& repository);

  entity create_entity();
  std::pair<entity, std::reference_wrapper<NodeComponent>> create_entity_node(std::string node_name
                                                                               = "");
...
class AssetLoader {
  ...
  size_t create_material(PbrMaterial& config, const std::string& name) const;
  ...
  MeshSurface create_surface(std::vector<GpuVertexPosition>& vertex_positions, std::vector<GpuVertexData>& vertex_data,
