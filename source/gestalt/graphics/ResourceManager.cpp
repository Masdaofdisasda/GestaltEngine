...
void ResourceManager::init(const Gpu& gpu, const std::shared_ptr<Repository> repository) {
  ...
  per_frame_data_buffer = create_buffer(sizeof(PerFrameData), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                        VMA_MEMORY_USAGE_CPU_TO_GPU);

  material_data.constants_buffer
      = create_buffer(sizeof(PbrMaterial::MaterialConstants) * max_materials,
...

  {
    ...
    memcpy(mappedData, vertex_positions.data(),
           sizeof(PbrMaterial::MaterialConstants) * max_materials);
...
void ResourceManager::init_default_data() {
  ...
  std::vector<PbrMaterial::MaterialConstants> material_constants(max_materials);
...
void ResourceManager::upload_mesh() {
  ...
  AllocatedBuffer staging
      = create_buffer(vertex_position_buffer_size + vertex_data_buffer_size + index_buffer_size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
  ...
  memcpy((char*)data + vertex_position_buffer_size, vertex_data.data(), vertex_data_buffer_size);
  // copy index buffer
  memcpy((char*)data + vertex_position_buffer_size + vertex_data_buffer_size, indices.data(), index_buffer_size);
  ...
  gpu_.immediate_submit([&](VkCommandBuffer cmd) {
    ...
    vkCmdCopyBuffer(cmd, staging.buffer, scene_geometry_.vertexPositionBuffer.buffer, 1, &vertex_positions_copy);
  ...
  writer.clear();
  writer.write_buffer(0, scene_geometry_.vertexPositionBuffer.buffer, vertex_position_buffer_size, 0,
...
void ResourceManager::write_material(PbrMaterial& material,
...
void ResourceManager::load_and_create_cubemap(const std::string& file_path, AllocatedImage& cubemap) {
  int w, h, comp;
