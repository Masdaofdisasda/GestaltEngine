...
namespace gestalt::graphics {
    ...
    void RenderPassBase::init(IGpu* gpu, ResourceManager* resource_manager, ResourceRegistry* registry,
    ...
    RenderPassDependency& RenderPassBase::get_dependencies() { return dependencies_; }

    void RenderPassBase::cleanup() {
      destroy();
    ...
    void RenderPassBase::begin_renderpass(VkCommandBuffer cmd) {
  ...
  void RenderPassBase::bind_descriptor_buffers(
  ...
  void RenderPassBase::begin_compute_pass(VkCommandBuffer cmd) {
      if (!dependencies_.buffer_dependencies.empty()) {
        std::vector<DescriptorBuffer*> buffers;
        buffers.resize(dependencies_.buffer_dependencies.size());
        for (const auto& buffer_dependency : dependencies_.buffer_dependencies) {
    ...
    PipelineBuilder RenderPassBase::create_pipeline() {
    ...
    void RenderPassBase::create_pipeline_layout() {
      ...
      VK_CHECK(vkCreatePipelineLayout(gpu_->getDevice(), &pipeline_layout_create_info, nullptr,
                                      &pipeline_layout_));
      resource_manager_->SetDebugName(get_name(), VK_OBJECT_TYPE_PIPELINE_LAYOUT,
                                      gpu_->getDevice(),
