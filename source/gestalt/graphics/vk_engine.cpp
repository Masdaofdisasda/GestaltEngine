...
void VulkanEngine::init_default_data() {
  ...
  std::optional<std::vector<std::shared_ptr<MeshAsset>>> assets = loadGltfMeshes(
      this, R"(..\..\assets\basicmesh.glb)");
  if (assets.has_value()) {
    testMeshes = assets.value();
  } else {
    fmt::print("Error when loading the gltf mesh");
    abort();
  }
}
...
void VulkanEngine::draw_geometry(VkCommandBuffer cmd) {
    ...
    VkDescriptorSet imageSet
        = get_current_frame()._frameDescriptors.allocate(_device, _singleImageDescriptorLayout);
    {
      DescriptorWriter writer;
