...
class SsaoFilterPass final : public RenderPass {
  ...
  ShaderPassDependencyInfo deps_ = {
      .read_resources
      = {{std::make_shared<DepthImageResource>("grid_depth", 1.0f)}},
      .write_resources
      = {{"color_ssao_filtered", std::make_shared<ColorImageResource>("color_ssao_filter", extent_)}}
  };
...
class SsaoBlurPass final : public RenderPass {
  ...
  ShaderPassDependencyInfo deps_ = {
      ...
      = {},
      .write_resources
      = {{"ssao_blurred_final", std::make_shared<ColorImageResource>("color_ssao_filtered", extent_)},
         {"ssao_blurred_intermediate", std::make_shared<ColorImageResource>("ssao_blur_y", extent_)}},
  };
...
class SsaoFinalPass final : public RenderPass {
  ...
  ShaderPassDependencyInfo deps_ = {
      ...
      .write_resources = {{"scene_ssao",
                           std::make_shared<ColorImageResource>("scene_ssao_final", 1.0f)}}};
