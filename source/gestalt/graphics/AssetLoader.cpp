...
void AssetLoader::init(const Gpu& gpu,
...
size_t AssetLoader::create_material(PbrMaterial& config,
...
void AssetLoader::import_metallic_roughness(
...
void AssetLoader::import_material(fastgltf::Asset& gltf, size_t& sampler_offset, size_t& image_offset, fastgltf::Material& mat) const {
...
void AssetLoader::optimize_mesh(std::vector<Vertex>& vertices,
...
size_t AssetLoader::create_mesh(std::vector<MeshSurface> surfaces, const std::string& name) const {
...
void AssetLoader::import_meshes(fastgltf::Asset& gltf, const size_t material_offset) {
  ...
  for (fastgltf::Mesh& mesh : gltf.meshes) {
    ...
    for (auto&& primitive : mesh.primitives) {
      ...
      MeshSurface surface = create_surface(vertex_positions, vertex_data, indices, std::move(meshlets));
