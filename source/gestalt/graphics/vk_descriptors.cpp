...
namespace gestalt::graphics {
    ...
    void descriptor_writer::write_buffer_array(
        int binding, const std::vector<VkDescriptorBufferInfo>& bufferInfos, VkDescriptorType type,
    ...
    void descriptor_writer::write_image(int binding, VkImageView image, VkSampler sampler,
    ...
    void DescriptorAllocator::init_pool(VkDevice device, uint32_t maxSets,
      ...
      for (pool_size_ratio ratio : poolRatios) {
        poolSizes.push_back(VkDescriptorPoolSize{
            .type = ratio.type, .descriptorCount = uint32_t(ratio.ratio * maxSets)});
      }
