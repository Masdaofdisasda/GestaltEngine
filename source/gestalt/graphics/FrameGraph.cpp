...
void FrameGraph::init(const Gpu& gpu, const ApplicationWindow& window,
                       const std::shared_ptr<ResourceManager>& resource_manager,
                       const std::shared_ptr<Repository>& repository,
                       const std::shared_ptr<ApplicationGui>& imgui_gui) {
  gpu_ = gpu;
  window_ = window;
  ...
  render_passes_.push_back(std::make_unique<DirectionalDepthPass>()); //TODO investigate sorting
...
void FrameGraph::create_resources() {
  ...
  for (auto& r : resource_map | std::views::values) {
    if (auto image_resource = std::dynamic_pointer_cast<ColorImageResource>(r)) {
      ...
      auto image = std::make_shared<AllocatedImage>(ImageType::kColor);
      image->imageExtent
      ...
      auto image = std::make_shared<AllocatedImage>(ImageType::kDepth);
      image->imageExtent
          = {image_resource->get_extent().width, image_resource->get_extent().height, 1};
      if (image->imageExtent.height == 0 && image->imageExtent.width == 0) {
        image->imageExtent.width
