...
void lighting_pass::execute(VkCommandBuffer cmd) {
  ...
  VkDescriptorSet descriptorSets[]
      = {resource_manager_->ibl_data.IblSet, descriptor_set_};
  ...
  resource_manager_->config_.lighting.screenSize = glm::vec2(color_image->getExtent2D().width,
                                                                 color_image->getExtent2D().height);
  resource_manager_->config_.lighting.invViewProj = inverse(resource_manager_->per_frame_data_.viewproj);
