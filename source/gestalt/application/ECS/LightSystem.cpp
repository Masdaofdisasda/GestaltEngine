...
namespace gestalt::application {
  ...
  void LightSystem::create_buffers() {
    ...
    light_data->view_proj_matrices = resource_allocator_->create_buffer(std::move(BufferTemplate(
        "view_proj_matrices", sizeof(GpuProjViewData) * GetMaxViewProjMatrices(),
        VK_DESCRIPTOR_TYPE_STORAGE_BUFFER | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT,
        VMA_MEMORY_USAGE_CPU_TO_GPU)));
    light_data->dir_light_buffer = resource_allocator_->create_buffer(std::move(BufferTemplate(
        "dir_light_buffer",
        sizeof(GpuDirectionalLight) * getMaxDirectionalLights(),
        VK_DESCRIPTOR_TYPE_STORAGE_BUFFER | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT,
        VMA_MEMORY_USAGE_CPU_TO_GPU)));
    light_data->point_light_buffer = resource_allocator_->create_buffer(std::move(BufferTemplate(
        "point_light_buffer", sizeof(GpuPointLight) * getMaxPointLights(),
        VK_DESCRIPTOR_TYPE_STORAGE_BUFFER | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT,
        VMA_MEMORY_USAGE_CPU_TO_GPU)));
  ...
  void LightSystem::fill_buffers() {
    ...
    light_data->descriptor_buffer
        ->write_buffer(0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
                       light_data->view_proj_matrices->get_address(),
                       sizeof(GpuProjViewData) * GetMaxViewProjMatrices())
        .write_buffer(1, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, light_data->dir_light_buffer->get_address(),
                      sizeof(GpuDirectionalLight) * getMaxDirectionalLights())
        .write_buffer(2, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, light_data->point_light_buffer->get_address(),
  ...
  void LightSystem::update(float delta_time, const UserInput& movement, float aspect) {
    ...
    vmaUnmapMemory(gpu_->getAllocator(), light_data->point_light_buffer->get_allocation());
    ...
    vmaUnmapMemory(gpu_->getAllocator(), light_data->view_proj_matrices->get_allocation());
