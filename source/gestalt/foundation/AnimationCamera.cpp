...
namespace gestalt::foundation {
  ...
  glm::vec3 ClipAngles(const glm::vec3& angles) {
    return {std::fmod(angles.x, 360.0f), std::fmod(angles.y, 360.0f),
  ...
  glm::vec3 AngleDelta(const glm::vec3& anglesCurrent,
  ...
  void MoveToCamera::update(float64 delta_seconds, const Movement& movement, CameraData& data) {
    auto& camera_data = std::get<AnimationCameraData>(data);
  ...
  glm::mat4 MoveToCamera::get_view_matrix(CameraData& data) const {
    const auto& camera_data = std::get<AnimationCameraData>(data);
